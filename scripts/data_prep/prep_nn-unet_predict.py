...
for file_path in glob.iglob(in_dir + "/*.*"):
    ...
    v_sizes = (
        round(float(v_sizes[0]), v_size_decimal),
    ...
    if v_size_tomo != v_size_model:
        print(
            "\t-Scaling input to tomogram from",
            v_size_tomo,
            "A to fit the model voxel size of",
            v_size_model,
            "A ...",
        )
        tomo = sp.ndimage.zoom(tomo, v_size_tomo / v_size_model)
    nrrd.write(
        imgs_ts_dir + "/tomo_" + str(tomo_id).zfill(3) + "_0000.nrrd", tomo
    )
    tomo_id += 1
