...
VOI_SHAPE = (
    ...
    512,
    256,
)  # (1856, 1856, 236) # (1856, 1856, 464) # (400, 400, 150) # (400, 400, 464) # (924, 924, 300) # vx
...
# Proteins list
PROTEINS_LIST = [
    "in_shrec/5MRC_10A.pns",
    "in_shrec/4V94_10A.pns",
    "in_shrec/4CR2_10A.pns",
    "in_shrec/1QVR_10A.pns",
    "in_shrec/1BXN_10A.pns",
    "in_shrec/3CF3_10A.pns",
    "in_shrec/1U6G_10A.pns",
    "in_shrec/3D2F_10A.pns",
    "in_shrec/2CG9_10A.pns",
    "in_shrec/3H84_10A.pns",
    "in_shrec/3GL1_10A.pns",
    "in_shrec/3QM1_10A.pns",
...
SURF_DEC = (
    None  # 0.9 # Target reduction factor for surface decimation (defatul None)
)
...
DETECTOR_SNR = [0.15, 0.25]  # 0.2 # None
MALIGN_MN = 1
MALIGN_MX = 1.5
MALIGN_SG = 0.2
...
for tomod_id in range(NTOMOS):
    ...
    for p_id, p_file in enumerate(PROTEINS_LIST):
        ...
        if protein.get_pmer_np() is not None:
            ...
            voi_voxels = voi.sum()
            protein_occ = (
                100 * (protein.get_pmer_np() * model_voxels) / voi_voxels
            )
        else:
        ...
        synth_tomo.add_network(
            net_sawlc, "Protein", lbl=p_id, code=protein.get_mmer_id()
        )
