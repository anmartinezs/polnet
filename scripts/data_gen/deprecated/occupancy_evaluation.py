...
VOI_SHAPE = (
    ...
    500,
    200,
)  # vx or a path to a mask (1-foreground, 0-background) tomogram
VOI_OFFS = (
    (4, 496),
    (4, 496),
    (4, 196),
...
MEMBRANES_LIST = (
    []
)  # ['in_mbs/sphere.mbs', 'in_mbs/ellipse.mbs', 'in_mbs/toroid.mbs' ]
...
MB_PROTEINS_LIST = (
    []
)  # ['in_10A/mb_5wek_10A.pms', 'in_10A/mb_4pe5_10A.pms', 'in_10A/mb_5ide_10A.pms',
...
TILT_ANGS = range(
    -60, 61, 3
)  # np.arange(-60, 60, 3) # at MPI-B IMOD only works for ranges
...
with open(OUT_DIR + "/occupancies.csv", "w") as file:
    writer = csv.DictWriter(
        file, fieldnames=["Target_Occupancy", "Occupancy", "Density", "Time"]
    )
    writer.writeheader()
...
for occ in RANGE_OCCUPANCIES:
    ...
    for p_id, p_file in enumerate(HELIX_LIST):
        ...
        if helix.get_type() == "mt":
            ...
            hold_br_vtp, hold_br_skel_vtp = (
                net_helix.get_branches_vtp(br_vtp),
                net_helix.get_branches_vtp(),
            )
            pp.add_label_to_poly(hold_br_vtp, entity_id, "Entity", mode="both")
            pp.add_label_to_poly(
                hold_br_skel_vtp, entity_id, "Entity", mode="both"
            )
    ...
    mb_occ = 100.0 * (mb_voxels / voi_voxels)
    ac_occ = 100.0 * (ac_voxels / voi_voxels)
    mt_occ = 100.0 * (mt_voxels / voi_voxels)
    pt_occ = 100.0 * (cp_voxels / voi_voxels)
    mp_occ = 100.0 * (mp_voxels / voi_voxels)
    ...
    print(
        "\t\t+Microtublues:", count_mts, "#, ", mt_den, "um**3, ", mt_occ, "%"
    )
    ...
    with open(OUT_DIR + "/occupancies.csv", "a") as file:
        writer = csv.DictWriter(
            file,
            fieldnames=["Target_Occupancy", "Occupancy", "Density", "Time"],
        )
        writer.writerow(
            {
