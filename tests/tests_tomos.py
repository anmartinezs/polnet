...
A_PMER_OCC = 0.1  # 5 # %
A_MIN_P_LEN = (
    17.7e4  # 5200e4 # Actin filament 17.7um according to 10.1083/jcb.130.4.909
)
A_HP_LEN = 720  # A
A_MZ_LEN = 50  # A
A_MZ_LEN_F = 0.2
...
MT_MMER_ISO = 0.9
MT_MMER_RAD = (
    40  # A according to http://web.physics.ucsb.edu/~jennyr/research1.html
)
MT_RAD = (
    100.5  # A according to http://web.physics.ucsb.edu/~jennyr/research1.html
)
MT_NUNITS = 13
MT_PMER_OCC = 0.1  # 5 # %
...
class TestTomos(TestCase):
    ...
    def test_build_tomo(self):
        ...
        voi[
            VOI_OFF : VOI_SHAPE[0] - VOI_OFF,
            VOI_OFF : VOI_SHAPE[1] - VOI_OFF,
            VOI_OFF : VOI_SHAPE[2] - VOI_OFF,
        ] = True
        ...
        mb_sph_generator = SphGen(
            radius_rg=(0.5 * param_rg[0], 0.5 * param_rg[1])
        )
        ...
        net_sawlc.insert_density_svol(
            model_mask, voi, VOI_VSIZE, merge="min", off_svol=off
        )
        net_sawlc.insert_density_svol(
            model, tomo, VOI_VSIZE, merge="max", off_svol=off
        )
        hold_gtruth_vtp = net_sawlc.get_skel(
            add_lines=False, verts_rad=GTRUTH_POINTS_RAD
        )
        ...
        model_surf = iso_surface(
            model, RIBO_MMER_ISO, closed=False, normals=None
        )
        ...
        write_mrc(
            net_sawlc.get_voi().astype(np.float32),
            STOMO_VOI_OUT,
            v_size=VOI_VSIZE,
        )
