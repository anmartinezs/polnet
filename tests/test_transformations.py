...
class TestTransformations(TestCase):
    ...
    def test_positions_rotations(self):
        ...
        df_filt = df[
            df["Density"].str.contains(den_file_name)
            & (df["Label"] == IN_LABEL_CODE)
        ]
        ...
        for row in df_filt.iterrows():
            ...
            q_rots = np.asarray(
                (row[1]["Q1"], row[1]["Q2"], row[1]["Q3"], row[1]["Q4"])
            )

            # Applying the transformations
            tomo_rot = affine.tomo_rotate(tomo_model, q_rots)
            utils.insert_svol_tomo(
                tomo_rot, tomo, center / VOI_VSIZE, merge="max"
            )
