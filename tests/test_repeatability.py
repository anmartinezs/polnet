...
class TestRepeatability(unittest.TestCase):
    ...
    def generate_membranes(self):
        ...
        param_rg = (
            self.mb_min_rad,
            np.sqrt(3) * max(self.voi_shape) * self.voi_vsize,
            self.mb_max_ecc,
        )
    ...
    def test_repeatability(self):
        ...
        np.testing.assert_array_equal(
            tomo1, tomo2, "Tomograms are not identical"
        )

        self.assertEqual(
            vtp1.GetNumberOfPoints(),
            vtp2.GetNumberOfPoints(),
            "VTPs have different number of points",
        )
        self.assertEqual(
            vtp1.GetNumberOfCells(),
            vtp2.GetNumberOfCells(),
        ...
        for i in range(vtp1.GetNumberOfPoints()):
            ...
            np.testing.assert_array_almost_equal(
                point1, point2, err_msg=f"Points at index {i} are not equal"
            )
...
class TestAllFeatures2Repeatability(unittest.TestCase):
    def setUp(self):
        ...
        self.light_params = {
            ...
            "PMER_TRIES": 50,
            "MEMBRANES_LIST": [
                os.path.join(self.data_dir, "in_mbs", "sphere.mbs")
            ],
            "HELIX_LIST": [
                os.path.join(self.data_dir, "in_helix", "actin.hns")
            ],
            "PROTEINS_LIST": [
                os.path.join(self.data_dir, "in_10A", "4v7r_10A.pns")
            ],
            "MB_PROTEINS_LIST": [],
            "SURF_DEC": 0.9,
            "TILT_ANGS": (-30, 31, 5),
    ...
    def compare_csv(self, file1, file2):
        ...
        columns_to_compare = [
            col for col in df1.columns if col not in ignore_columns
        ]
    ...
    def test_repeatability(self):
        ...
        for file in key_files:
            ...
            if file.endswith(".mrc"):
                ...
                if len(points1) == 0 and len(points2) == 0:
                    print(
                        f"Both VTP files {file} are empty. Skipping comparison."
                    )
        ...
        for file, result in results.items():
            print(
                f"{file}: {'Passed' if result == True else 'Failed' if result == False else result}"
            )
    ...
    def test_seed_sensitivity(self):
        all_features2(
            **self.light_params, OUT_DIR=f"{self.out_dir}/seed1", random_seed=1
        )
        all_features2(
            **self.light_params, OUT_DIR=f"{self.out_dir}/seed2", random_seed=2
        )
        ...
        self.assertFalse(
            self.compare_arrays(arr1, arr2),
            "Output should be different with different seeds",
        )
